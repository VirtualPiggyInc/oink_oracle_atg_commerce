    ...
    --%>
        ...
        <dsp:getvalueof var="vpTransStatus"
        bean="/atg/commerce/ShoppingCart.last.paymentGroups[0].authorizationStatus[0].vpTransactionStatus"/>
        </c:when>
        <c:otherwise>
        ...
        <p>
        <span>Your order <dsp:valueof bean="ShoppingCart.last.id"/> is saved and is pending parent approval. It will be
        processed when approved.</span>
        </p>
        </c:when>
        <c:otherwise>
        <p>
        ...
        <dsp:input bean="BillingFormHandler.registerAccountSuccessURL" type="hidden" value="${contextPath}
        /myaccount/profile.jsp"/>
        <%-- If registration fails, redisplay this page with errors shown --%>
        <dsp:input bean="BillingFormHandler.registerAccountErrorURL" type="hidden"
